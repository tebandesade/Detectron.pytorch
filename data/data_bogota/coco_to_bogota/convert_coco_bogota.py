...
import coco_to_bogota_id as cs
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Convert a COCO pre-trained model for use with Bogota')
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default='bogota_to_coco', type=str)
    parser.add_argument(
...
def convert_coco_blobs_to_bogota_blobs(model_dict):
    for k, v in model_dict['blobs'].items():
        if v.shape[0] == NUM_COCO_CLS or v.shape[0] == 4 * NUM_COCO_CLS:
            ...
            cs_blob = convert_coco_blob_to_bogota_blob(
...
def convert_coco_blob_to_bogota_blob(coco_blob, convert_func):
...
def remove_momentum(model_dict):
    for k in list(model_dict['blobs']):
...
def load_and_convert_coco_model(args):
    with open(args.coco_model_file_name, 'rb') as f:
        print(args.coco_model_file_name)
        model_dict = pickle.load(f,encoding='latin1')
    remove_momentum(model_dict)
    convert_coco_blobs_to_bogota_blobs(model_dict)
    return model_dict
